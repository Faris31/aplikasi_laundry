    ...
    <style>
        * {
        ...
        body {
            ...
            background: linear-gradient(125deg, #676971 0%, #584df8 100%);
            min-height: 100vh;
            color: #4a4a4a;
        }
...
<body>
    <div class="container">
                ...
                <form id="transactionForm">
                    ...
                    <div class="form-group">
                        ...
                        <select id="customerName" required>
                            ...
                            <option value="{{ $customer->id }}" data-name="{{ $customer->customer_name }}" data-phone="{{ $customer->phone }}"
        ...
        <div style="text-align: center; margin-top: 20px">
            ...
            <button class="btn btn-success" style="margin: 0 10px">
                <a href="{{ route('transaksi.index') }}" style="text-decoration: none; color: white;">
    ...
    <script>
        ...
        async function processTransaction() {
            ...
            try {
                const res = await fetch("{{ route("transaksi.OrderStore") }}", {
                    method: "POST",
                    headers: {
            ...
            const receiptHtml = `
                <div class="receipt">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        `
                            <div class="receipt-item">
                              ...
                              item.service.includes("Sepatu") ? "pasang" : item.service.includes("Karpet") ? "m²" : "kg"})</span>
                    ...
                    <div class="receipt-total">
                        <div class="receipt-item">
                            <span>TOTAL:</span>
                            <span>Rp ${parseFloat(transaction.total ?? 0).toLocaleString()}</span>
                        </div>
                    </div>
        ...
        function updateStats() {
            ...
            console.log('updateStats:', transactions);
                    ...
                    ${transactions
                      .map(
                        ...
                        <div class="transaction-item">
                            ...
                            <p>🛍️ ${transaction.items.map((item) =>
                                  `${item.service} - ${item.weight}${
                                    item.service.includes("Sepatu") ? "pasang" : item.service.includes("Karpet") ?"m²" : "kg"}`).join(", ")}</p>
                            <p>💰 Rp ${transaction.total.toLocaleString()}</p>
        ...
        function manageServices() {
            const servicesHtml = `
                ...
                <div style="text-align: center; margin-top: 20px;">
                    <a class="btn btn-primary" href="{{ route('service.index') }}">
                        ➕ Tambah Layanan Baru
                    </a>
                </div>
            `;
        ...
        async function saveStatusUpdate(transactionId) {
            let url = `{{ route('transaksi.pickupLaundry', ':id') }}`.replace(':id', transactionId);
            ...
            try {
                ...
                console.error("Tidak berhasil menyimpan data :", error)
            }
        }
        ...
        //     const month = String(date.getMont()+1).padStart(2,'0');
        ...
        async function loadDataTransaction(){
            try {
                const res = await fetch("{{ route('transaksi.getAllDataOrders') }}");
                ...
                console.error("Gagal Membuat Data Transaksi:", error)
            }
        }
